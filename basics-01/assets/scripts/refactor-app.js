...
function getCurrentAndPreviousNumber() {
  const enteredNumber = getUserNumberInput();
  const prevCalculatedNumber = currentNumber;

  return { enteredNumber, prevCalculatedNumber };
}

function add() {
  const { enteredNumber, prevCalculatedNumber } = getCurrentAndPreviousNumber();

  // * input tag의 value 속성은 항상 문자열(string) 형태로 처리된다. 문자열을 숫자 자료형으로 변경해줘야 한다.
